...
CREATE INDEX IX_office_employee_employee_id          ON office_employee(employee_id);
CREATE INDEX IX_office_employee_office_id            ON office_employee(office_id);

COMMENT ON TABLE office_employee                    IS 'отношение многие ко многим для офисов и сотрудников';
COMMENT ON COLUMN office_employee.employee_id       IS 'внешний ключ - сотрудник';
COMMENT ON COLUMN office_employee.office_id         IS 'внешний ключ - офис';
...
ALTER TABLE doc         ADD FOREIGN KEY (employee_id)         REFERENCES employee(id);
ALTER TABLE doc         ADD FOREIGN KEY (doc_list_id)         REFERENCES doc_list(id);

CREATE INDEX IX_employee_id                 ON doc(employee_id);
...
COMMENT ON COLUMN doc.employee_id           IS 'внешний ключ - принадлежность сотруднику';
